...
export function normalizeOfficialStudioFan(fan: string | number): OfficialStudioFan {
  const value = String(fan).trim().toLowerCase();
  if (value === "1" || value === "part" || value === "part_cooling") return "part";
  if (value === "2" || value === "aux" || value === "auxiliary" || value === "left_auxiliary") return "auxiliary";
  if (value === "10" || value === "right_aux" || value === "right_auxiliary") return "right_auxiliary";
  if (value === "3" || value === "chamber" || value === "exhaust") return "chamber";
  throw new Error("Unsupported X2D fan. Use part, auxiliary, right_auxiliary, chamber, 1, 2, 3, or 10.");
}

export async function setFanSpeedViaOfficialBambuStudio(fan: string | number, speed: number) {
  ...
  const scriptPath =
    process.env.BAMBU_OFFICIAL_STUDIO_CONTROL_SCRIPT ||
    path.resolve(moduleDir, "..", "scripts", "bambu-studio-fan-control.applescript");

  const { stdout } = await execFileAsync(
    "/usr/bin/osascript",
    [scriptPath, normalizedFan, String(roundedSpeed)],
...
export async function setTemperatureViaOfficialBambuStudio(component: string, temperature: number) {
  if (process.platform !== "darwin") {
    throw new Error("The official Bambu Studio control bridge is available only on macOS.");
  }
  ...
  const moduleDir = path.dirname(fileURLToPath(import.meta.url));
  const scriptPath =
    process.env.BAMBU_OFFICIAL_STUDIO_TEMPERATURE_SCRIPT ||
    path.resolve(moduleDir, "..", "scripts", "bambu-studio-temperature-control.applescript");

  const { stdout } = await execFileAsync(
    "/usr/bin/osascript",
    [scriptPath, normalizedComponent, String(roundedTemperature)],
    { timeout: 30_000, maxBuffer: 64 * 1024 }
  );
  ...
  return {
    status: "success",
    route: "official_bambu_studio",
    component: normalizedComponent as OfficialStudioTemperatureComponent,
